...
- Create: `src/main.tsx`, `src/App.tsx`, `src/api.ts`, `src/store.ts`, `src/format.ts`, `src/Sidebar.tsx`, `src/Splash.tsx`
...
export function Splash({ steps, error }: { steps: Step[]; error: string }) {
  return (
    <div className="splash">
      <img className="splash-art lighten" src={logo} alt="" width={220} height={220} />
      <div className="splash-name">Meridian</div>
      {error ? (
        <div className="splash-error">
          <div>{error}</div>
          <div className="splash-hint">Tried {apiBase() || 'no API address'}</div>
        </div>
      ) : (
        <ul className="splash-steps">
          {steps.map((s) => (
            <li key={s.label} className={s.done ? 'on' : ''}>
              <i className={`ph ${s.done ? 'ph-check-circle' : 'ph-circle-dashed'}`} />
              <span>{s.label}</span>
            </li>
          ))}
        </ul>
...
Import the artwork as `import logo from '../assets/logo.png'` so Vite hashes and
bundles it. Style it from Nocturne tokens only: `var(--color-bg)` ground, the
wordmark in `var(--font-heading)` at weight 500, done steps in
`var(--color-text)` and pending ones in `var(--color-neutral-400)`, and the
`.lighten` wrapper class the design system specifies for photographic art on a
dark ground. No spinner: the three steps are the progress, and they are honest
because the frontend is the thing making those calls.
...
its UI was served over HTTP and took real time to arrive. Here the UI is on the
