...
- Create: `assets/logo.png` (the circular artwork: splash art and app icon)
- Create: `assets/banner.png` (the wide 2:1 artwork: README header and release notes)
...
**The artwork is the app icon at every size**, decided by the user: the same
circular logo in the splash, the taskbar, the app menu and the window. No
separate simplified mark. `cargo tauri icon` generates the whole set from it.

Worth knowing rather than acting on: at 32 pixels the watercolour detail
dissolves and it reads as a purple disc. That is the right colour and the right
silhouette, which is most of what a 32 pixel icon does, so it is accepted.
...
- [ ] **Step 5: Generate the icon set and check it where it is seen**
...
Confirm it appears in the app menu, the window switcher and the window's title
bar. The same artwork serves every size, by decision; nothing to swap out.

- [ ] **Step 6: Write README.md**

Open it with `assets/banner.png`, the wide 2:1 artwork, as the header image, and
use the same banner at the top of each release's notes.
