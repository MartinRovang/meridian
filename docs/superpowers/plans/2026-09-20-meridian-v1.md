...
- Create: `src/main.tsx`, `src/App.tsx`, `src/api.ts`, `src/store.ts`, `src/format.ts`, `src/Sidebar.tsx`
...
- Produces:
  ...
  - `store.ts`: a Zustand store with `{ base, token, state, screen, error, load(), setScreen() }`
...
`src/App.tsx` renders the sidebar plus the current screen, a stale-quotes
...
```json
{
  ...
  "app": {
    "windows": [
      { "label": "main", "title": "Meridian", "width": 1400, "height": 900, "center": true }
    ],
    "security": {
...
Generate icons from the design's accent mark: `cargo tauri icon path/to/logo.png`.
...
Expected: a real Equinor price, a real value in NOK, a real day change.
